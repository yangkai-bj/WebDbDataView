...
function getMultiGraphOption(configs, container, myChart, dataset) {
    ...
    function getGrids(source, types, cols, layout) {
        ...
        if (layout === "auto") {
            ...
            let border = 10;
            let sep = 5;
            let width = (100 - border * 2 - sep * (cols - 1)) / cols;
            let height = (100 - border * 2 - sep * (lines - 1)) / lines;
            for (let i = 1; i < source.dimensions.length; i++) {
                let line = Math.ceil(i / cols) - 1;
                let col = (i - 1) % cols;
                let grid = {};
                grid = {
                    left: (border + width * col + (col == 0 ? 0 : sep)) + "%",
                    top: (border + height * line + (line == 0 ? 0 : sep)) + "%",
                    right: (100 - border - (col == 0 ? 0 : sep) - width * col) + "%",
                    bottom: (100 - border - (line == 0 ? 0 : sep) - height * line) + "%",
                    center: [(border + (col == 0 ? 0 : sep) + width * col + width / 2) + "%", (border + (line == 0 ? 0 : sep) + height * line + height / 2) + "%"],
                    width: width + "%",
                    height: height + "%",
                    containLabel: true
                };
    ...
    let grids = getGrids(source, types, cols, configs.multiGraphGrids.value);
...
function getPieOption(configs, container, myChart, dataset) {
    ...
    for (let c = 1; c < source.dimensions.length; c++) {
        let serie = {
            ...
            data: source.getItems(source.dimensions[c]),
            colorBy: configs.pieColorby.value,
            radius: [(configs.pieRoseType.value == "radius" ? "0%" : configs.pieInRadius.value), configs.pieOutRadius.value],
            roseType: configs.pieRoseType.value,
            selectedMode: configs.pieSelectedMode.value,
            label: configs.richTextLabel.value.toBoolean() ? getPieRichText(configs, colors) : {
                show: configs.pieLabelDisplay.value.toBoolean(),
    ...
    let h = parseInt((100 - toPoint(configs.gridTop.value) - toPoint(configs.gridBottom.value)) / lines);
    let w = (100 - toPoint(configs.gridLeft.value) - toPoint(configs.gridRight.value)) / groupWith;
...
function getCategoryLineForGauge(container, width, height, dataset, configs) {
    ...
    function getMinMax(col) {
        let value = {min: +Infinity, max: -Infinity};
        for (let i = 0; i < dataset["data"].length; i++) {
            let row = dataset["data"][i];
            let v = row[columns[col]].value;
            if (value.min > v)
                value.min = v;
            if (value.max < v)
                value.max = v;
    ...
    for (let i = 0; i < dataset["data"].length; i++) {
        ...
        for (let c = 1; c < columns.length; c++) {
            ...
            let left = (toPoint(configs.gridLeft.value) + (100 - toPoint(configs.gridLeft.value) - toPoint(configs.gridRight.value)) / 2);
            let top = (toPoint(configs.gridTop.value) + (100 - toPoint(configs.gridTop.value) - toPoint(configs.gridBottom.value)) / 2);
...
function getWordCloud(container, width, height, dataset, configs) {
    ...
    function init() {
        ...
        let lines = parseInt(series.length / groupWith + 0.5);
        let height = parseInt((100 - toPoint(configs.gridTop.value) - toPoint(configs.gridBottom.value)) / lines);
        let width = (100 - toPoint(configs.gridLeft.value) - toPoint(configs.gridRight.value)) / groupWith;
