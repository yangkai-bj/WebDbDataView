...
var __ECHARTS__ = {
     ...
     configs: {
         ...
         LabelShowFor3D: {name: "3D图形显示标签", value: "NO", options: ["YES", "NO"], type: "select"},
...
function getMapConfig() {
    ...
    let region = echarts.getMap("china").geoJson.features;
    for (var i = 0; i < region.length; i++) {
        localmap.options.add(new Option(region[i].properties.name, region[i].properties.name));
    }
    try {
    ...
    localmap.onchange = function () {
        ...
        setUserConfig("localMap", this.value);
    };
