var __CONFIGS__ = {
    STORAGE: {
        DATABASES: "__WEB_DATAVIEW_DATABASES__",
        SCRIPTS: "__WEB_DATAVIEW_SCRIPTS__",
        DATASET: "__WEB_DATAVIEW_DATASET__",
        CONFIGS: "__WEB_DATAVIEW_USER_CONFIGS__",
    },
    DATABASES: [],
    CURRENT_DATABASE: {name: null, value: null, service: null, state: -1},
    TABLES: [],
...
const __VERSION__ = {
    name: "Web DataView for MySQL Database",
    main: "WebMySQLDataView.js",
    ...
    url: __SYS_LOGO_LINK__.link.gethub.decode(),
    tel: __SYS_LOGO_LINK__.tel.decode(),
    email: __SYS_LOGO_LINK__.email.decode(),
    logo: __SYS_LOGO_LINK__.image,
    notes: "需MySQL_Query_Analysis_Server(Version:2021/04/16)提供WS服务.",
    helps: {
        create_database_connect: "创建数据库(MySQL)连接。<span style='font-size:50%'><li>连接名称</li><li>WS服务地址</li><li>用户信息</li></span>",
        delete_database_connect: "删除当前数据库连接。",
...
var __XMLHTTP__ = {
    ...
    hook: function (dom, timeout) {
        ...
        function startTime(index) {
            ...
            __XMLHTTP__.elements[index].innerHTML = "时间:" + (__XMLHTTP__.time == null ? "" : __XMLHTTP__.time.format("yyyy-MM-dd hh:mm")) + " 更新频率:" + (userTimelength.value + userTimelength.unit);
    ...
    certificate: function (byServer) {
        ...
        let scripts = [
            {name: "主程序", src: "WebMySQLDataView.js", type: "text/javascript", element: "script", load: false},
            {name: "主程序", src: "WebWCClient.js", type: "text/javascript", element: "script", load: false},
            {name: "主程序", src: "themes/default.css", type: "text/css", element: "link", load: false},
            {name: "主程序", src: "WebMySQLDataView.css", type: "text/css", element: "link", load: false},
            {name: "主程序", src: "UI-B.js", type: "text/javascript", element: "script", load: false},
            ...
            {name: "codemirror", src: "codemirror/mysql.js", type: "text/javascript", element: "script", load: false},
            {
                name: "codemirror",
...
function transferDataToDatasetRow(structure,row,rowid) {
    //####################################
    // 用于数据导入的行数据转换
    // 读取当前数据表结构
    let _row = {};
        ...
        if (types.length <= row.length) {
            for (let c = 0; c < types.length; c++) {
                ...
                switch (_type) {
                    case "float":
                        _type = "number";
                        _value = Number.parseFloat(row[c]);
                        _format = floatFormat;
                        _align = "right";
                        break;
                    case "decimal":
...
function viewDatabases() {
    let message = __LOGS__.viewMessage("读取数据库连接列表...");
    try {
        ...
        for (let database in __CONFIGS__.DATABASES) {
            let li = document.createElement("li");
            li.className = "database-list";
            let a = document.createElement("a");
            a.className = "list";
            a.innerText = database;
            a.setAttribute("index", database);
            a.id = database;
            a.draggable = "true";
            ...
            a.onclick = function () {
                ...
                login(this.getAttribute("index"), false);
            };
            li.appendChild(a);
            let dul = document.createElement("ul");
            dul.id = "ul-db-" + database;
...
function login(name,reset) {
    let database = __CONFIGS__.DATABASES[name];
    if (__CONFIGS__.CURRENT_DATABASE.name != name || reset) {
        __CONFIGS__.CURRENT_DATABASE.name = name;
        __CONFIGS__.CURRENT_DATABASE.value = database;
        __CONFIGS__.CURRENT_DATABASE.service = getWebSocket(__CONFIGS__.CURRENT_DATABASE.value.Serverhost);

        let waitingfor = setInterval(function () {
            Timer()
        }, 1000);

        function Timer() {
            if (__CONFIGS__.CURRENT_DATABASE.state == 1 && __CONFIGS__.CURRENT_DATABASE.value.Localhost != null) {
                try {
                    let msg = {
                        user: __CONFIGS__.CURRENT_DATABASE.value.User,
                        password: __CONFIGS__.CURRENT_DATABASE.value.Password.decode(),
                        address: __CONFIGS__.CURRENT_DATABASE.value.Localhost
                    };
                ...
                __LOGS__.viewMessage("连接数据库异常,请重试!", true);
            }
            Stop();
        }
            ...
            for (let i = 0; i < sqls.length; i++) {
                ...
                if (sql != "") {
                    try {
                        ...
                        let msg = {
                            type: messageType.selectSql,
                            eventid: eventid,
                            message: sql.encode()
                        };
                        //__CONFIGS__.CURRENT_DATABASE.service.send(JSON.stringify(msg));
                        //连续发送时造成WS服务阻塞,间隔1秒发送一次
                        setTimeout(sendSQL, (i + 1) * 1000, msg);
                    } catch (e) {
                        __LOGS__.viewError("auto", e);
                    }
                }
...
function executeFunction(title) {
    ...
    if (selection.trim() != "") {
        ...
        let yRange = __ECHARTS__.configs.mathFunctionYRange.value.toArray([-100, 100], ",");
...
function initConfigs() {
    ...
    if (checkStorage()) {
        ...
        UI.alert.show("提示","当前浏览器不支持Local Storage,请使用Chrome或Edge浏览器.");
        checked = false;
    }
...
function initMenus() {
    ...
    try {
        ...
        crdb.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.create_database, crdb, crdb.id, "auto"));
        let help_crdb = $("help-create-database");
        crdb.onclick = help_crdb.onclick = function () {
            MySQL.createDatabase("auto", function (args, values) {
                let storage = window.localStorage;
        ...
        UI.tooltip(crdb, "创建数据库连接");

        let rmdb = document.createElement("div");
        rmdb.className = "button";
        rmdb.innerText = "删除";
        rmdb.id = "delete_database_connect";
        rmdb.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.drop_database, rmdb, rmdb.id, "auto"));
        rmdb.onclick = function () {
            if (__CONFIGS__.CURRENT_DATABASE.name == null || __CONFIGS__.CURRENT_DATABASE.name == "") {
                UI.alert.show("提示", "请选择数据库连接.");
                return;
            }
            UI.confirm.show("注意", "确定要删除数据库连接 " + __CONFIGS__.CURRENT_DATABASE.name + " 吗?", "auto", function () {
                if (checkStorage()) {
                    if (__CONFIGS__.CURRENT_DATABASE.value != null) {
                        ...
                        let list = {};
                        for (let db in dbs) {
                            if (db != __CONFIGS__.CURRENT_DATABASE.name) {
                                list[db] = dbs[db];
                            }
                        }
                        storage.setItem(__CONFIGS__.STORAGE.DATABASES, JSON.stringify(list));
                        viewDatabases();
                        __CONFIGS__.CURRENT_DATABASE.name = "";
        ...
        //测试功能键，使用时打开。
        dbinfo.id = "test-function";
        dbinfo.onclick = function () {
            UI.QRCode("auto",
                "春江花朝秋月夜，往往取酒还独倾。" +
                "座中泣下谁最多，江州司马清衫湿。", 400, 400);
        };
        dbstools.appendChild(dbinfo);
        UI.tooltip(dbinfo, "程序测试");
        ...
        reset.className = "charButton";
        reset.innerHTML = "&#10227";//⟳
        reset.id = "reset-database";
        ...
        reset.onclick = function () {
            if (__CONFIGS__.CURRENT_DATABASE.name != null) {
                ...
                sleep(1000);
                login(__CONFIGS__.CURRENT_DATABASE.name, true);
            }
        };
        dbstools.appendChild(reset);
        UI.tooltip(reset, "重置当前数据库连接");
        ...
        crtb.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.create_table, crtb, crtb.id, "auto"));
        let help_crtb = $("help-create-table");
        crtb.onclick = help_crtb.onclick = function () {
            MySQL.createTable("auto", null, function (values) {
                try {
                    let eventid = getEventIndex();
                    let result = {
                        eventid: eventid,
        ...
        importtb.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.import, importtb, importtb.id, "auto"));
        let help_importtb = $("help-import-data");
        importtb.onclick = help_importtb.onclick = function () {
            MySQL.import.start("auto");
        };
        ...
        exConstr.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.table_construct, exConstr, exConstr.id, "auto"));
        exConstr.onclick = function () {
            try {
                let eventid = getEventIndex();
                let result = {
                    eventid: eventid,
                    title: [__CONFIGS__.CURRENT_TABLE.name],
                    columns: [],
                    data: [],
                    total: 0,
                    sql: "table construct",
                ...
                let msg = {
                    ...
                    table: __CONFIGS__.CURRENT_TABLE.name,
                    type: __CONFIGS__.CURRENT_TABLE.type
                };
                let msgs = {
                    type: messageType.tableFullInfo,
                    eventid: eventid,
                    message: JSON.stringify(msg).encode(),
                };
                __CONFIGS__.CURRENT_DATABASE.service.send(JSON.stringify(msgs))
        ...
        UI.tooltip(exConstr, "获取数据表结构");

        let rmtb = document.createElement("div");
        rmtb.className = "button";
        rmtb.innerText = "删除";
        rmtb.id = "drop_table";
        rmtb.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.drop_table, rmtb, rmtb.id, "auto"));
        ...
        newsql.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.create_sql, newsql, newsql.id, "auto"));
        ...
        opensql.id = "open_sql";
        opensql.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.open_sql, opensql, opensql.id, "auto"));
        ...
        saveto.innerText = "保存";
        saveto.id = "save_sql";
        saveto.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.save_sql, saveto, saveto.id, "auto"));
        ...
        loadfile.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.load_sql, loadfile, loadfile.id, "auto"));
        ...
        saveas.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.unload_sql, saveas, saveas.id, "auto"));
        ...
        backup.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.unload_sql, backup, backup.id, "auto"));
        ...
        execsql.appendChild(__SYS_IMAGES__.getHelpButton(__SYS_IMAGES__.execute_sql, execsql, "execute_sql", "auto"));
                ...
                if (__SQLEDITOR__.configs.codeMirrorMode.value == "text/html")
                    ...
                    if (params != null) {
                        ...
                        UI.prompt.show("输入脚本参数", params, "auto", function (args, values) {
                            ...
                            if (__SQLEDITOR__.title != null) {
                                let title = __SQLEDITOR__.title;
                                if (__SQLEDITOR__.configs.codeMirrorMode.value == "text/x-mysql")
                                    execute(title);
        ...
        tofull.id = "editer_to_full_green";
        ...
        toconfigs.innerText = "┅";
        toconfigs.style.fontSize = "140%";
        toconfigs.style.cssFloat = "right";
        toconfigs.id = "dataset-configs";
        ...
        getecharts.id = "dataset-to-echarts";
...
function getEchartsClock() {
    try {
        if (__DATASET__.result.length == 0) {
            ...
            let configs = JSON.stringify(__ECHARTS__.getConfigs());
